...
class GRUBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        context = torch.cat([last_step_feat, global_avg_feat], dim=-1)  # [batch, 2 * d_model]

        h = F.relu(self.head_fc1(context))
        h = self.head_dropout1(h)
        h = F.relu(self.head_fc2(h))
        h = self.head_dropout2(h)
        out = self.out_proj(h)  # [batch, horizon]
        return out
...
if __name__ == '__main__':
    print("=" * 65)
    print("GRU PyTorch FULL HPO (Cho et al. 2014; Chung et al. 2014)")
    print("=" * 65)
    ...
    study = optuna.create_study(
        ...
        direction="minimize",
        study_name="16_hpo_gru_pytorch_full"
    )
    ...
    output_json = "16_hpo_gru_pytorch_best_params.json"
