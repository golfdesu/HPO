...
# ==============================================================================
# Hyperparameter Optimization (HPO) for Model 26: N-BEATS
...
# - n_layers: [2, 3, 4]
# - stack_types: ["generic", "interpretable", "hybrid"]
...
class NBeatsBlock(nn.Module):
    ...
    def __init__(
        ...
        if basis_type == "trend":
            ...
            self.register_buffer('basis_b', torch.pow(t_b, p))  # [lookback, poly+1]
            self.register_buffer('basis_f', torch.pow(t_f, p))  # [horizon, poly+1]
            ...
            self.register_buffer('basis_b', torch.cat([cos_b, sin_b], dim=1))  # [lookback, 2*H]
...
def objective(trial):
    ...
    stack_config = trial.suggest_categorical("stack_config", ["generic", "interpretable", "hybrid"])
    ...
    if stack_config == "generic":
        stack_types = ["generic", "generic", "generic"]
    elif stack_config == "interpretable":
...
if __name__ == '__main__':
    print("=" * 70)
    print("🚀 Model 26 HPO: N-BEATS Study")
    print("=" * 70)
    ...
    study = optuna.create_study(
        ...
        direction="minimize",
        study_name="26_hpo_nbeats_pytorch"
    )

    study.optimize(objective, n_trials=50)

    print("\n" + "=" * 70)
    print("🏆 BEST HYPERPARAMETERS FOUND FOR N-BEATS:")
    print("=" * 70)
    ...
    output_json = "26_hpo_nbeats_pytorch_best_params.json"
    ...
    best_data = {
        "model_name": "26_hpo_nbeats_pytorch",
