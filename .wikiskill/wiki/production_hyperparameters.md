...
| **16-S4D** | **S4D Baseline** | Rank 1 | 0.003200 | $d=32, d_{\text{state}}=32, L=1, \text{drop}=0.10, \text{lr}=4.57\times 10^{-4}, B=64$ | Global numerical minimizer by $>6.7\%$ with canonical $B=64$, 0.10 dropout, and $d_{\text{state}}=32$. |
| **20** | **MFT (Multi-Feature Transformer)** | Rank 1 | 0.004036 | $d=64, H=8, d_{\text{ff}}=256, L=1, \text{dec}=128, \text{drop}=0.10, \text{lr}=9.91\times 10^{-4}, B=128$ | Dominant numerical minimizer with canonical 0.10 dropout, 4x FFN expansion, 8 multi-scale heads, and mini-batch 128. |
| **21** | **CNN-LSTM-Transformer** | Rank 1 | 0.003285 | $\text{cnn}=64, k=3, \text{lstm}=128, L_{\text{lstm}}=2, d=128, H=8, L_{\text{tfm}}=2, \text{drop}=0.05, \text{lr}=1.61\times 10^{-4}, B=128$ | Deep hierarchical feature extraction combining 2-layer stacked LSTM with 2-layer 8-head self-attention. |
| **22** | **FEDformer** | Rank 1 | 0.003116 | $d=64, \text{modes}=24, d_{\text{ff}}=128, L=2, k=49, \text{drop}=0.20, \text{lr}=1.85\times 10^{-4}, B=64$ | Global numerical minimizer with 0.20 dropout, diurnal moving average filter (kernel 49, ~24.5h), and 24 Fourier modes. |
| **23** | **TCN Baseline** | Rank 1 | 0.003095 | $\text{channels}=128, L=4, k=5, \text{drop}=0.20, \text{lr}=9.58\times 10^{-4}, B=64$ | Outperforms candidates by >4.2% margin; full lookback coverage (121-step receptive field across 4 dilated layers with kernel 5). |
| **24** | **N-HiTS Baseline** | Rank 1 | 0.003056 | $\text{hidden}=256, L=2, \text{pooling}=[16,8,2], \text{drop}=0.20, \text{lr}=7.10\times 10^{-4}, B=128$ | Dominates candidates by >2.6% margin; hierarchical multi-rate pooling [16, 8, 2] captures multi-frequency load variations. |
